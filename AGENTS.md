...
- Dartvel-annotated generation inputs must be private and begin with `_` (for example `@DVModel() class _User`). Application code must reference the generated public API (`User`, `User.Form(...)`, generated widgets, generated routes), not the annotated input declaration. Until full body lowering exists, private `@DVPage`, `@DVFunctionalWidget`, and `@DVBackendFunction` function inputs must use expression bodies; public annotated functional widget inputs are hard errors.
...
- Job handlers are `@DVJob.handler()`; do not add a standalone `@DVJobHandler` annotation, which would collide with the `DVJobHandler` runtime typedef. `@DVJob` classes and their handlers are private generation inputs, and handlers use expression bodies until body lowering exists.
...
- Terminal rendering rides the community `jiahaog/flt` embedder, which uses Flutter's Custom Embedder API from Rust and renders through the Kitty graphics protocol with an ANSI fallback. Upstream is a research project that runs apps in development rather than producing distributable binaries; supplying that, and anything else Dartvel needs, is the fork's job rather than a reason to wait. Terminal rendering is opt-in at build time — the `-cli` suffix or a `dartvel.terminal` key in `pubspec.yaml` — and an application that opts into neither must link no terminal code at all.
...
- An embedder's Flutter can also be too **old**, and there are two floors, not one. The lower is Dart >= 3.9, which `dartvel_shelf`'s native-asset build hook needs for `code_assets`. The binding one is higher and comes from the UI layer: `dartvel_flutter` depends on `mix: ^2.0.0`, which requires **Dart >= 3.11.0**. Check the higher floor — reasoning that an embedder "clears the Dart 3.9 floor" is what wrongly recorded webOS as merely unproven when its bundled Dart 3.10.9 cannot resolve dependencies at all. State which wall a target actually hits — engine ceiling, `code_assets` floor, or `mix` floor — rather than assuming it is the engine.
