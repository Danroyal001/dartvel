...
| `windows` | ⚠️ Two bugs found, one fixed | The hang is diagnosed and gone. It was the **native asset hook**, not the Flutter build: `cargo build` was unbounded, so a cold-cache compile of ~200 crates was indistinguishable from a wedge, and the CLI never started — which is why `--build-timeout` could not help. With the hook bounded and its output streamed, the Rust runtime now compiles in **2m55s**. That revealed a **second, separate** bug the hang had been masking: `ffigen` failed with `Could not find the pub cache. No LOCALAPPDATA environment variable exists.` Fixed by deriving `PUB_CACHE` for hook children. Not yet re-run. See [Windows](#windows) |
