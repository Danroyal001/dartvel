...
""";
    ...
    // Write merged env to .dartvel/.env for envied
    final mergedEnvFile = File(p.join(root, '.dartvel', '.env'));
    if (!mergedEnvFile.parent.existsSync()) {
      mergedEnvFile.parent.createSync(recursive: true);
    }
    final sbMerged = StringBuffer();
    for (final e in envMap.entries) {
      sbMerged.writeln('${e.key}=${e.value}');
    }
    mergedEnvFile.writeAsStringSync(sbMerged.toString());
    ...
    sbEnv.writeln('import \'package:envied/envied.dart\';');
    sbEnv.writeln('');
    sbEnv.writeln('part \'env.g.dart\';');
    sbEnv.writeln('');
    sbEnv.writeln('@Envied(path: \'.dartvel/.env\', obfuscate: true)');
    sbEnv.writeln('abstract class Env {');
    for (final key in publicEnv.keys) {
      sbEnv.writeln('  @EnviedField(varName: \'$key\')');
      sbEnv.writeln('  static final String $key = _Env.$key;');
    }
    sbEnv.writeln('}');
    sbEnv.writeln('');
    ...
    // Write to env.dart (env.g.dart will be generated by build_runner)
    File(p.join(libClientDir.path, 'env.dart'))
