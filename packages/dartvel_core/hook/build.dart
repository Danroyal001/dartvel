...
Future<void> main(List<String> args) async {
  await build(args, (input, output) async {
    ...
    if (cargoCheck == null || cargoCheck.exitCode != 0) {
      _say(
          'dartvel_core hook: cargo not found on system, skipping Rust compilation.');
      return;
    }

    if (!rustDir.existsSync()) {
      _say('dartvel_core hook: rust directory missing, skipping.');
      return;
    }
    ...
    if (cbindgenCheck == null || cbindgenCheck.exitCode != 0) {
      _say(
          'dartvel_core hook: cbindgen not found, skipping native build. '
    ...
    if (!wantsCodeAsset) {
      _say(
          'dartvel_core hook: target does not support native code assets; skipping.');
      return;
    }
    ...
    if (tripleInfo == null) {
      _say(
          'dartvel_core hook: no native artifact target configured for '
...
Future<ProcessResult?> _runBounded(
  ...
  try {
    ...
    final code = await process.exitCode.timeout(timeout, onTimeout: () {
      stderr.writeln(
          'dartvel_core hook: $executable ${arguments.join(' ')} exceeded '
