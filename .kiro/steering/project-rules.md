...
- `mix` is forked as `dartvel_mix` for a different reason than the embedders: it is not a build target but a dependency whose SDK constraint **sets Dartvel's own floor**, because it is the styling layer under `DVBox` and `DVText`. Upstream targets the oldest Dart it can, which is right for a general-purpose package and wrong for a framework that needs one floor users must satisfy. Every Dartvel package, every fork and the example declare the **same** Dart and Flutter floor, matching what Dartvel ships with; a test fails the build when they disagree or when the declared floor drops below what a dependency actually requires.
